...
// Guard: if the Supabase CDN script failed to load (ad-blocker, offline CDN,
// wrong <script> order, etc.) fail loudly instead of throwing a confusing
// "supabase is not defined" error further down.
if(typeof supabase==='undefined'||!supabase.createClient){
  console.error('[Supabase] SDK not loaded — check that the @supabase/supabase-js <script> tag is present and loads BEFORE app.js.');
}
...
let user=null;
let _authInitDone=false; // avoids a double/late render racing the INITIAL_SESSION event

// Single, authoritative auth listener (handles email login, Google OAuth,
// AND the initial page-load/refresh session restore).
// Supabase fires 'INITIAL_SESSION' once, right after this listener is
// attached, with whatever session is currently in localStorage — this is
// the correct way to restore a session on refresh (more reliable than a
// separate getSession() call racing the 'load' event).
_sb.auth.onAuthStateChange(async(ev,s)=>{
  try{
    if((ev==='INITIAL_SESSION'||ev==='SIGNED_IN'||ev==='TOKEN_REFRESHED'||ev==='USER_UPDATED')&&s&&s.user){
      await loadProfile(s.user);
    }else if(ev==='SIGNED_OUT'||(ev==='INITIAL_SESSION'&&!s)){
      user=null;updateUI();
    }
  }catch(e){
...
// Safety net: if, for any reason, INITIAL_SESSION never fires (very old
// supabase-js version pulled from the CDN, network hiccup, etc.) fall back
// to an explicit getSession() so the UI never gets stuck looking "logged
// out" indefinitely.
window.addEventListener('load',async()=>{
  ...
  try{
    ...
    if(!_authInitDone){
      if(session&&session.user){await loadProfile(session.user);}else{updateUI();}
    }
  }catch(e){console.error('[auth] getSession fallback failed:',e);}
});
...
async function handleRegister(){
  ...
  if(error){const em=error.message||'';showE('re',em.includes('already')?'البريد مسجل بالفعل — سجّل دخولك':(em||'حدث خطأ أثناء إنشاء الحساب'));console.error('[signup] supabase error:',error);return;}
